...
function selectDistanceBand(distanceMeters) {
  return DISTANCE_BANDS.find((band) => distanceMeters <= band.max) ?? DISTANCE_BANDS[DISTANCE_BANDS.length - 1];
}
...
async function main() {
  ...
  const referenceCoords = {
    lat: Number.isFinite(options.referenceOverride.lat) ? options.referenceOverride.lat : metadata.apartment_reference_coords?.lat ?? DEFAULT_REFERENCE_COORDS.lat,
    lng: Number.isFinite(options.referenceOverride.lng) ? options.referenceOverride.lng : metadata.apartment_reference_coords?.lng ?? DEFAULT_REFERENCE_COORDS.lng
  };
